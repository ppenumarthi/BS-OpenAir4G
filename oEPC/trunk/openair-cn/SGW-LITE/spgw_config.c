...
#if defined (ENABLE_USE_GTPU_IN_KERNEL) //Specific Modificationed by nfvepc, brought from bottom to top.
        if (snprintf(system_cmd, 256,
              ...
              for (i = 0; i < num; i++) {
                  ...
                  if (astring != NULL) {
                      ...
                      if (inet_pton(AF_INET, atoken, buf_in_addr) == 1) {
                          ...
                          //if (snprintf(system_cmd, 128, "ip route add %s/%s via %s dev %s", //--Specific Modification by nfvepc
                          if (snprintf(system_cmd, 128, "ip route add %s via %s dev %s", //-- Specific Modification by nfvepc
                                       astring,
                                       //atoken2, //-- Specific Modification by nfvepc
                                       inet_ntoa(in_addr_var),
                                       config_pP->sgw_config.ipv4.sgw_interface_name_for_S1u_S12_S4_up) > 0) {
